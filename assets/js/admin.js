...
jQuery(document).ready(function ($) {
	...
	function formatChildRow(d) {
		var html = '<div class="child-row-content"><table class="child-details">';

		function getFieldHtml(label, value, fallback) {
			return '<tr><td><strong>' + label + ':</strong></td><td>' + (value ? value : '<em>' + fallback + '</em>') + '</td></tr>';
		}

		function formatLinks(idsStr, baseUrl, suffix) {
			if (!idsStr) {
				return null;
			}
			return idsStr.split(', ').map(function (id) {
				return '<a href="' + baseUrl + id + suffix + '" target="_blank">' + id + '</a>';
			}).join(', ');
		}

		html += getFieldHtml('Description', d.description, 'No description');
		html += getFieldHtml('Confirmation Message', d.confirmation, 'No confirmation message');
		html += getFieldHtml('Account Message', d.account_message, 'No account message');

		html += getFieldHtml('Protected Category IDs', formatLinks(d.protected_categories, 'term.php?taxonomy=category&tag_ID=', '&post_type=post'), 'No protected categories');
		html += getFieldHtml('Protected Page IDs', formatLinks(d.protected_pages, 'post.php?post=', '&action=edit'), 'No protected pages');
		html += getFieldHtml('Protected Post IDs', formatLinks(d.protected_posts, 'post.php?post=', '&action=edit'), 'No protected posts');
